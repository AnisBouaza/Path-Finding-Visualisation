  ...
  for(int a = 0; a<taille_grille[0]; a++)
  {
   ...
   while(!trouver && !file.isEmpty())
    ...
    for( int count = 0; count < 4; count++)
    {
      ...
      {
        Voisins[count].cout = Courant.cout + grille[Voisins[count].x][Voisins[count].y]+ (int)Math.sqrt((int)Math.pow(Objectif.x- Voisins[count].x,2) * (int)Math.pow(Objectif.y- Voisins[count].y,2));
